...
func parseRequest(conn net.Conn) (map[string]interface{}, error) {
	// This function would be moved to a separate http package in a real project:
	// for example, https://github.com/go-chi/chi/blob/master/pkg/netutil/conn.go
	// Implementation details remain the same as before
	// This is kept here for simplicity
	return handlers.ParseRequest(conn)
}
